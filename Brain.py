...
class FeatAnal(nn.Module):
    def __init__(self, action_num):
        super().__init__()
        self.net = nn.Linear(in_features=512, out_features=action_num)
...
class FeatValAnal(nn.Module):
    def __init__(self):
        super().__init__()
        self.net = nn.Linear(in_features=512, out_features=1)
...
class Brain(nn.Module):
    def __init__(self, action_num, dueling=False):
        super().__init__()
        if dueling:
            self.policy = DuelingNetwork(action_num)
            self.target = DuelingNetwork(action_num)
        else:
            self.policy = CNNNetwork(action_num)
            self.target = CNNNetwork(action_num)
