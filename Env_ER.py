'''
This is originated from the Gym Documentation.

First wrapped with AtariProcessing,
then wrapped with TransformReward and TransformObservation,
last wrapped with FrameStack
...
from gym.wrappers import TimeLimit, FrameStack, TransformReward, TransformObservation
...
class FireReset(gym.Wrapper):
    ...
    def __init__(self, env):
        super().__init__(env)
        self.fire_reset = (env.unwrapped.get_action_meanings()[1] == 'FIRE') and (len(env.unwrapped.get_action_meanings()) >= 3)

    def step(self, action):
        """gym.Env step function."""
        obs, reward, done, info = self.env.step(action)
        if done and info['dont reset'] and self.fire_reset:
...
class EpisodicLife(gym.Wrapper):
    ...
    def step(self, action):
        ...
        info['dont reset'] = False
        if lives < self._lives and lives > 0:
            info['dont reset'] = not self._was_real_done
            done = True
        self._lives = lives
        return obs, reward, done, info
